...
class Snake:
    ...
    def grow(self, former_tail_x, former_tail_y, tail_direction):
        ...
        snake_tail = SnakeCell(
            former_tail_x, former_tail_y, self.cte.SNAKE_SIZE, self.cte.SNAKE_SIZE
        )
...
class Food:
    ...
    def new_food(self):
        ...
        self.position = (
            X + (self.x + 1 / 2) * SNAKE_SIZE,
            Y + (self.y + 1 / 2) * SNAKE_SIZE,
        )
        self.radius = self.cte.RADIUS
        self.hit_box = pygame.Rect(
            X + self.x * SNAKE_SIZE, Y + self.y * SNAKE_SIZE, SNAKE_SIZE, SNAKE_SIZE
        )
...
class SnakeGame:
    ...
    def play_step_by_step(self):
        ...
        while not terminated:
            action = (
                input("Enter action (z/q/s/d to move, k to kill/quit): ")
                .strip()
                .lower()
            )
            if action == "k":
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    self._snake = Snake(
                        *self.cte.INIT_SNAKE_COO, game_size=self.game_size
                    )
                    self._food.new_food()
                    self.done = False
                    self.score = 0
    ...
    def _get_snake_coo(self, x, y):
        ...
        x, y = (
            (x - self.cte.X) // self.cte.SNAKE_SIZE,
            (y - self.cte.Y) // self.cte.SNAKE_SIZE,
        )
        return x, y
    ...
    def draw_score(self, display, font, score):
        ...
        score_position = (
            self.cte.SCORE_COO[0],
            self.cte.SCORE_COO[1] - text.get_height() // 2,
        )
...
def main():
    ...
    parser.add_argument(
        "-g", "--game_size", type=int, default=10, help="Size of the game grid (NxN)."
    )
