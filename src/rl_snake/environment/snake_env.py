...
class SnakeEnv(BaseSnakeEnv):
    ...
    def __init__(self, game_size: int = 30, fast_game: bool = True):
        ...
        self._last_distance = self.manhattan_distance(head=head, food=food) # update each step
        self.distance_to_food = self.manhattan_distance(head=head, food=food) # update each time snake eats food
    ...
    def feature_gen(self, raw_obs, info):
        ...
        time_to_food = np.array([min(info["n_steps"] - self.food_steps, 2*self.distance_to_food) / self.distance_to_food])  # normalize to [0, 2]
