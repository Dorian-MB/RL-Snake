...
class SnakeProgressCallback(BaseCallback):
    ...
    def _get_postfix_dict(self, ep_info_buffer=None):
        ...
        if ep_info_buffer and len(ep_info_buffer) > 0:
            ...
            recent_episodes = list(
                deque(ep_info_buffer, maxlen=min(20, len(ep_info_buffer)))
            )
    ...
    def _on_step(self) -> bool:
        ...
        if self.pbar and (
            self.num_timesteps - self.last_update >= self.update_freq
            or self.num_timesteps == self.model._total_timesteps
        ):
            # Update progress
